...
function buildEditor(cmd) {
  ...
  if (cmd) {
    ...
    meta.title = 'Usage count updates each time the command runs';
  ...
  saveBtn.addEventListener('click', () => {
    ...
    if (activeId) {
      ...
      commands.push({ id: newId, name, hint, script, createdAt: Date.now(), uses: 0 });
      activeId = newId;
    }
    save(() => {
      showToast('✅ Saved!');
      renderList();
      // Rebuild so the meta line appears (or refreshes its "edited" date).
